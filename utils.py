...
def get_graph(batch):
    # Branch 1: Handling inference (no labels passed)
    if len(batch) != 2:
        graph = batch
        ...
        from_idx = torch.as_tensor(graph.from_idx, dtype=torch.long)
        to_idx = torch.as_tensor(graph.to_idx, dtype=torch.long)
        graph_idx = torch.as_tensor(graph.graph_idx, dtype=torch.long)
        
        return node_features, edge_features, from_idx, to_idx, graph_idx
        
    # Branch 2: Handling training/evaluation (graph + labels passed)
    else:
        graph, labels = batch
